...
class AddressDeliveryNonOrganization extends AbstractDelivery implements ShippingProcessorInterface
{
    ...
    private function calculateCODShippingPrice($object)
    {
        $price = $this->getNovaposhtaApi()->getApi()->request('InternetDocument', 'getDocumentPrice', [
            ...
            */
             ...
             * эти поля
             */
            'RecipientAddressName' => $object->getCurrentUserAddress()->getStreet() ? $this->streetRepository->getByRef($object->getCurrentUserAddress()->getStreet()) : 'Тестова',
            'RecipientHouse' => $object->getCurrentUserAddress()->getBuilding()?:1,
            'RecipientFlat' => $object->getCurrentUserAddress()->getFlat()?:1,
    ...
    private function calculateNonCODShippingPrice($object)
    {
        $price = $this->getNovaposhtaApi()->getApi()->request('InternetDocument', 'getDocumentPrice', [
            ...
            'PaymentMethod' => 'Cash', //$object->getPaymentMethod(), по какой-то причине НП не принимает NonCash
            'Description' => __('Order') . $object->getOrderId(),
            'RecipientCityName' => $object->getCityRecipientString(),
            /*
            * следующие поля требуют дополнительной работы с импортом городов. нужно по имени населенного пункта найти область, район и т.д.
